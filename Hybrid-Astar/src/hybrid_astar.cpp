...
bool HybridAStar::isReachedGoal(const Pose2D& pos, const Pose2D& goal) {
  if (std::abs(pos.x() - goal.x()) < _goal_tolerance_dist && std::abs(pos.y() - goal.y()) < _goal_tolerance_dist && std::abs(pos.theta() - goal.theta()) < _goal_tolerance_theta) {
    return true;
  }
  return false;
}
...
double HybridAStar::length(const std::vector<Pose2D>& path) {
  ...
  for (int i = 1; i < path.size(); ++i) {
    s += std::sqrt(std::pow(path[i].x() - path[i - 1].x(), 2) + std::pow(path[i].y() - path[i - 1].y(), 2));
  }
  return s;
}
